...
public class AttackManager_C001 : AttackManager
{
    ...
    public void Skill3()
    {
        ...
        if (s3_upgraded)
        {
            ...
            (_statusManager as PlayerStatusManager).SetSPChargeRateAll(300);
            _statusManager.OnSpecialBuffDelegate?.Invoke
                (UI_BuffLogPopManager.SpecialConditionType.AutoChargeRateUp.ToString());

            s3_tween = DOVirtual.DelayedCall(20, () =>
            {
                (_statusManager as PlayerStatusManager).ResetSPChargeRate();
            },false);
