...
public class NpcInput: PlayerInput
{
    ...
    protected PlayerOnewayPlatformEffector _groundSensor;
    ...
    //public ActorController.OnTask OnMoveFinished;
    protected Coroutine currentMoveCoroutine;
    protected Coroutine subMoveRoutine;
    ...
    public bool subMoveRoutineIsRunning => subMoveRoutine != null;
    ...
    protected Action<bool> OnMoveFinished;
    ...
    protected virtual void Start()
    {
        ...
        _aStar = new AStar(ac.defaultGravity, ac.jumpforce, ac.movespeed);
        _aStar.Init(mapInfo);
        ...
        //_groundSensor.OnGroundEnter += OnFallToGround;
        ...
        _groundSensor = GetComponentInChildren<PlayerOnewayPlatformEffector>();
    }
        ...
        if (!isAction)
        {
            if (actionMode == ActionMode.FollowMode)
            {
                ...
                if (Math.Abs(currentTarget.transform.position.x - transform.position.x) > maxAttackActiveDistance ||
                    Math.Abs(currentTarget.transform.position.y - transform.position.y) > jumpHeight)
                {
            ...
            else if (actionMode == ActionMode.AttackMode)
            {
                ...
                if (currentTarget != null && currentTarget != playerGameObject)
                {
                    ...
                    else
                    {
                        actionMode = ActionMode.FollowMode;
                        currentTarget = null;
                        if (currentMoveCoroutine != null && subMoveRoutine == null)
                        {
    ...
    public virtual IEnumerator MoveTowardsTarget(GameObject target, float arriveDistance, float maxFollowTime)
    {
        ...
        while (maxFollowTime > 0)
        {
            ...
            if (subMoveRoutine == null && ac.anim.GetBool("isGround") == false)
            {
                if (currentTarget == null)
                ...
                if (jumptime > 0 && currentTarget.transform.position.x - transform.position.x > 1)
                {
                    ...
                    subMoveRoutine = StartCoroutine(StruggleInAir(targetPlatform, jumptime));
                    yield return new WaitUntil(() => subMoveRoutine == null);
                }
            }
            ...
            // 检查NPC和目标是否处于同一个平台上
            if (BasicCalculation.CheckOnSameRaycastPlatform(target, gameObject))
            {
                // 计算两者之间的x轴距离
            ...
            else
            {
                ...
                foreach (var platform in path)
                {
                    ...
                    subMoveRoutine = StartCoroutine(MoveToPlatform(platform));
    ...
    protected virtual IEnumerator StruggleInAir(APlatformNode node, int jumpTimeLeft)
    {
        ...
        try
        {
            currentPlatformCollider = _groundSensor.GetLastAttachedGroundInfo().GetComponent<Collider2D>();
        }
        catch
        {
            currentPlatformCollider = _groundSensor.GetCurrentAttachedGroundCol();
        }
    ...
    protected float[] GetRunUpInfo(float distanceY, Collider2D from, Collider2D to)
    {
        ...
        if (distanceY >= jumpHeight)
        {
            ...
            var freeHangTime = Mathf.Sqrt(2 * (jumpHeight * 2 - distanceY) / (ac.defaultGravity * 10));
            runUpDistance = ac.movespeed * (jumpAscentTime * 2 + freeHangTime);
            jumpTimes = 2;
        }
        else
        {
            var singleJumpLimit = jumpAscentTime * ac.movespeed;
            ...
            if (gapDistance > singleJumpLimit && distanceY >= 0)
            {
                ...
                var freeHangTime = Mathf.Sqrt(2 * (jumpHeight * 2 - distanceY) / (ac.defaultGravity * 10));
                runUpDistance = ac.movespeed * (jumpAscentTime * 3 + freeHangTime);
                jumpTimes = 2;
            }
            else if (gapDistance <= singleJumpLimit && distanceY >= 0)
            {
                //跳跃一次，因为距离够远。
                var freeHangTime = Mathf.Sqrt(2 * (jumpHeight - distanceY) / (ac.defaultGravity * 10));
                runUpDistance = ac.movespeed * (jumpAscentTime + freeHangTime);
                jumpTimes = 1;
            ...
            {
                //自由落体的时间
                var freeHangTime = Mathf.Sqrt(2 * (-distanceY) / (ac.defaultGravity * 10));
                var hangDistance = ac.movespeed * freeHangTime;
                var jumpHangTime = Mathf.Sqrt(2 * (jumpHeight - distanceY) / (ac.defaultGravity * 10));
                var jumpDistanceAfterSingleJump = ac.movespeed * (jumpAscentTime + jumpHangTime);
                var jumpDistacneAfterDoubleJump = ac.movespeed * (jumpAscentTime * 3 + jumpHangTime);
    ...
    protected APlatformNode GetAccessiblePlatforms(int jumpTimeLeft)
    {
        ...
        if (currentNode != null)
        {
            if (Platform.CanReach(transform.position, currentNode.platform,
                    ac.defaultGravity, ac.jumpforce, ac.movespeed, jumpTimeLeft))
            {
                return currentNode;
            }
        }
    ...
    protected List<APlatformNode> GetAccessiblePlatforms()
    {
        ...
        {
            if (Platform.CanReach(transform.position, pltform,
                    ac.defaultGravity, ac.jumpforce, ac.movespeed))
            {
                var distance = Mathf.Min(Vector2.Distance(pltform.rightBorderPos, currentTarget.transform.position),
                    Vector2.Distance(pltform.leftBorderPos, currentTarget.transform.position));
    ...
    public void SendForcedEvadeMessageDuringEvade(Collider2D col)
    {
        ...
        if (actionMode == ActionMode.EvadeMode && isMove > 0)
        {
            if (subMoveRoutine != null)
                StopCoroutine(subMoveRoutine);
            isMove = 0;
    ...
    protected bool CanReachSafelyInTime(Platform targetPlatform)
    {
        ...
        else
        {
            if (BasicCalculation.CheckRaycastedPlatform(gameObject).bounds.max.y < targetPlatform.height)
            {
                ...
                {
                    var outpoint = ray.point; //离开攻击范围的点
                    var averageVerticalSpeed = jumpHeight / jumpAscentTime;
            ...
            else
            {
                ...
                if (ray.collider != null)
                {
                    ...
                    var outTime = Mathf.Sqrt(2 * fallHeight / ac.defaultGravity);
                    if (outTime > ray.collider.GetComponent<EnemyAttackHintBar>().warningTimeLeft)
                        return false;
                }
            }
        ...
        )
        {
            ...
            if (safePointInAir1 < 999f)
            {
                underRay1 = BasicCalculation.GetPositionOfAttackEnemyBelow(
                    new Vector2(safePointInAir1, transform.position.y + jumpHeight - 1));
                safePointInAir = safePointInAir1;
                outPoint = new Vector2(safePointInAir, jumpHeight - 1);
            }
            else
            {
                underRay1 = BasicCalculation.GetPositionOfAttackEnemyBelow(
                    new Vector2(safePointInAir2, transform.position.y + 2 * jumpHeight - 1));
                safePointInAir = safePointInAir2;
                outPoint = new Vector2(safePointInAir, 2 * jumpHeight - 1);
            }
    ...
    protected Tuple<float, float> GetJumpTimeWhenAvoidAttackUnderfeet(EnemyAttackHintBar hintBar, float highestPointY)
    {
        ...
        if (needAscendDistance >= jumpHeight)
        {
            //一段跳躲不掉，只能二段跳
            var needAscendOnDoubleJump = needAscendDistance - jumpHeight;
            ...
            var maxAvoidTime = (jumpAscentTime - ascendOverAttackTime) * 2;
            var avoidNeedTime = jumpAscentTime + ascendOverAttackTime;
            ...
            {
                result = new Tuple<float, float>(avoidNeedTime - 0.1f, avoidNeedTime - 0.1f + jumpAscentTime);
                return result;
            }
        }
        else
        {
            ...
            var maxAvoidTime = (jumpAscentTime - ascendOverAttackTime) * 2 + (jumpAscentTime * 2);
            var minAvoidTime = maxAvoidTime - jumpAscentTime * 2;
            if (avoidNeedTime >= warningTimeLeft || maxAvoidTime <= attackLastTime)
            {
                return result;
            }
            ...
            {
                result = new Tuple<float, float>(avoidNeedTime - 0.1f, avoidNeedTime - 0.1f + (jumpAscentTime - ascendOverAttackTime) * 2);
                return result;
            }
        }
    ...
    protected float Get2ndJumpTimeWhenAvoidAttackUnderfeet(float height, float duration)
    {
        ...
        {
            return jumpAscentTime;
        }
        else
        {
            //跳跃高度超过height所需的时间
            var ascendOverAttackTime = Mathf.Sqrt(2 * height / ac.jumpforce);
            var safeTimeBySingleJump = 2 * (jumpAscentTime - ascendOverAttackTime);
            if (duration < safeTimeBySingleJump)
                return -1;
            else return jumpAscentTime + safeTimeBySingleJump;
        }
    ...
    public bool ReceiveEvadeSignal(float height, float duration, Collider2D col)
    {
        ...
        print(height);
        print(duration);
        print(jumpAscentTime);

        if (height < 0 || duration > jumpAscentTime * 2)
            return false;
        ...
        if (currentMoveCoroutine != null)
        {
            if (actionMode == ActionMode.FollowMode)
            {
                subMoveRoutine = StartCoroutine(AvoidJump(0, jumpTime2ND));
                return true;
            }
